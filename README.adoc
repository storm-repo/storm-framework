...
Dirty checking is only applied when updates are executed through an EntityRepository. Manual SQL updates,
bulk statements, or custom queries bypass dirty checking entirely and may leave in-memory entities stale.
...
Update modes influence how dirty information is interpreted and how UPDATE statements are generated. They primarily
...
Storm compares the current entity instance with the state that was observed when the entity was read in the same
transaction. If no difference is detected according to the configured dirty checking strategy, the UPDATE is skipped
entirely. If any difference is detected, Storm issues an UPDATE that includes all mapped columns.
...
In this mode, Storm compares individual fields against the values observed when the entity was read and generates an
UPDATE statement that includes only the modified columns, as determined by the configured dirty checking strategy.
This can reduce write amplification and limit lock scope.

These improvements are best-effort and situational. Field-level dirty checking can improve concurrency characteristics
when different transactions modify different fields. It does not guarantee correctness. Optimistic locking is still
required to reliably detect conflicting updates.
...
By default, Storm uses instance-based dirty checking. A field is considered dirty as soon as its reference changes.
