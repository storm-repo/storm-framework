...
Nested record components are treated as a single unit during dirty checking. Storm compares only top-level entity
fields and does not inspect individual fields inside embedded records.
...
Every call to update() results in an UPDATE statement that writes all mapped columns except the primary key. Storm
does not compare the current entity state with any previously observed state.
...
UpdateMode.ENTITY enables entity-level dirty checking.

Storm compares the current entity against the state that was observed when the entity was read in the same
transaction. The comparison is performed according to the configured dirty checking strategy.

If no difference is detected, the UPDATE is skipped entirely. If any difference is detected, Storm issues an UPDATE
that includes all mapped columns.
...
UpdateMode.FIELD enables field-level dirty checking.

In this mode, Storm compares individual fields against the values that were observed when the entity was read and
generates an UPDATE statement that includes only the modified columns, as determined by the configured dirty
checking strategy.
...
Field-level dirty checking is enabled per entity using the @DynamicUpdate annotation.
...
By default, Storm uses instance-based dirty checking. A field is considered unchanged when its reference is the same
as the one observed at read time. A field is considered dirty as soon as its reference changes. Entity-typed fields
are compared by identifier.

This approach provides predictable and bounded performance characteristics and avoids potentially expensive value
comparisons for large objects such as strings or collections.

Value-based dirty checking can be enabled explicitly. In this mode, fields are compared using semantic equality after
a fast reference check. A field is only considered dirty when its value has changed.
...
Field-level updates can produce multiple SQL statement shapes, depending on which fields are modified. To prevent
...
limit, Storm automatically falls back to a full-column update for that entity. This ensures that UpdateMode.FIELD
