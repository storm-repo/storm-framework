 ...
 *
 * The API is compiled against the engine but does not carry it: applications add it as a runtime dependency,
 * and the starters and the Gradle plugin do so on their behalf. Without that dependency the engine's absence
 * would first surface as a `NoClassDefFoundError` at the point a template runs a statement, so the entry points
 * that create a template check for it and report what is missing instead.
 *
 * @since 1.14
 */
internal object Engine {

    private const val ENGINE_CLASS = "st.orm.core.template.ORMTemplate"

    private val present: Boolean = try {
        Class.forName(ENGINE_CLASS, false, Engine::class.java.classLoader)
        true
