...
<div class="art">
  ...
  <p>ST/ORM makes a different choice at the root, the one behind <a class="tlink" href="/blog/entities-should-be-values">treating entities as plain data</a>. An entity is a value, fully there when you receive it. A field typed as its entity was loaded in the same query, through a join. A field you want to defer is a Ref, which holds the foreign key and nothing else, and loading it is a call you write in your own code. There is no proxy, because there is nothing to stand in for. There is no session for the value to outlive, because the value never belonged to one. You can return it, serialize it, cache it, or hand it to another thread, and none of that can produce a LazyInitializationException, because the conditions that create one are gone.</p>
