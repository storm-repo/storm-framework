...
- Added resolving a `Ref` as part of the query: `select().fetch(User_.city, User_.city.country)` selects the referenced table's columns in place of the foreign key column, so the reference comes back loaded and `Ref.fetch()` returns without querying. The record type is unchanged, and reference identity, equality and `unload()` are unaffected. The plan is prefix-closed, so a deeper path resolves the references it passes through, while a reference the plan does not name stays a foreign key column. A reference is a to-one foreign key, so resolving one widens the row without multiplying it, and a cycle stays bounded by the depth the path names. A nullable reference keeps its outer join. Paths that cross no reference, references to sealed types, and queries that do not select a record are rejected with a descriptive error. `Ref.getOrThrow()` reads a reference the query resolved without querying, and fails when it did not, so a query that stopped resolving it is reported where the assumption was made rather than degrading into one query per row.
- Fixed the value a reference metamodel reports being declared non-null: a reference metamodel is generated per target type, so one class serves nullable and non-null foreign keys alike, and a nullable foreign key holds no reference. The annotation processor now declares that value nullable, matching KSP.
- Fixed JSON columns in the metamodel: a converted column is addressed by its stored type (`String` for `@Json`) while value extraction keeps the declared type. The annotation processor previously substituted the stored type for both and generated a metamodel that did not compile, KSP ignored the annotation, and the runtime metamodel mistook a converted record field for an inline record. All three now agree.
...
- Changed: the `groupBy` and `orderBy` metamodel overloads resolve a path to the columns a predicate on that path uses. A foreign key contributes its foreign key column(s) on the referencing table, once, without joining the referenced table; an inline record contributes its component columns. Previously the core API expanded a foreign key to every column of the referenced table (joining it, and listing the key columns of a compound-key target twice), while the Kotlin and Java 21 APIs rejected compound foreign keys and inline records with "Expected exactly one column". All three APIs now share the same expansion, and descending order applies `DESC` to every expanded column.
- Changed: the primary key reached through a foreign key resolves to the foreign key column(s) on the referencing table for entity foreign keys as well, matching references: `Visit_.pet.id` names `visit.pet_id` in predicates, templates, `groupBy` and `orderBy` alike, without joining the referenced table (a compound key expands to every foreign key column). A single component of a compound key reached through a foreign key still resolves in the referenced table, which is joined on demand. Generated metamodels now flatten by the documented contract: an inline record expands into its component columns and every other node, entity nodes included, flattens to itself, so generated and reflection-built metamodels agree.
