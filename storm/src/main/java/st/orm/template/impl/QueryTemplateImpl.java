...
class QueryTemplateImpl implements QueryTemplate {
    ...
    public <T extends Record, R> QueryBuilder<T, R, ?> subquery(@Nonnull Class<T> fromType, @Nonnull Class<R> selectType, @Nonnull StringTemplate template) {
        return new QueryBuilderImpl<>(this, fromType, selectType, template, true);
    }
    ...
    protected <T extends Record, ID> Model<T, ID> createModel(@Nonnull Class<T> type, boolean requirePrimaryKey) throws PersistenceException {
        ...
        return (Model<T, ID>) MODEL_CACHE.computeIfAbsent(type, _ -> {
            List<Column> columns = Stream.of(type.getRecordComponents())
                    ...
                    .toList();
            String tableName;
            DbName dbName = REFLECTION.getAnnotation(type, DbName.class);
            if (dbName != null) {
                tableName = dbName.value();
        ...
        try {
            ...
            if (foreignKey) {
                ...
                if (componentType.isRecord()) {
                    if (!primaryKey) {
                        if (REFLECTION.isAnnotationPresent(component, DbName.class)) {
                            // @Inline is implicitly assumed.
                            throw new SqlTemplateException(STR."DbName annotation is not allowed for @Inline records: \{component.getDeclaringRecord().getSimpleName()}.\{component.getName()}");
                        }
                    }
