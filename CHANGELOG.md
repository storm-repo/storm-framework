...
- Added write sets (`orm.writeSet()`, Kotlin also `orm.writeSet { }`): one write action over a mixed-type collection of entities, ordered by foreign key dependencies and batched per type, with generated keys propagated by instance identity. `insert` and `upsert` extend the passed entities with their insertion closure of unsaved referenced entities; `update` and `remove` write exactly what is passed. Entities go in as varargs or any `Iterable`; `AndFetch` variants return the persisted state.
