...
Refs are a powerful feature provided by Storm ORM for efficiently managing entity relationships. A Ref serves as a
...
[source,java]
----
List<User> users = ORM(dataSource).entity(UserRole.class)
    .selectRef(User.class)
    .where(RAW."\{role}")
    .getResultList();
----
...
directly to a `Ref<City>` within the `GroupedByCity` record.
