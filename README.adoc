...
In addition to Spring’s declarative model, Storm for Kotlin provides *full programmatic transaction control* via APIs that work with both:

* **Suspend functions** (coroutine-friendly, non-blocking database access)
* **Regular (non-suspend) code** (blocking style)

* All common propagation models (`REQUIRED`, `REQUIRES_NEW`, `NESTED`, etc.)
* **Isolation level** — Control concurrency behavior and visibility of uncommitted changes.
* **Read-only mode** — Hint to the database and transaction manager that no data modifications will occur.
* **Timeout** — Specify the maximum duration a transaction may run before being rolled back.
* **Context switching** — Offload CPU-bound work to another thread or dispatcher while keeping the same transaction active.
...
* *Context switching*: Within any transactional scope, you can switch threads or dispatchers
(e.g., `withContext(Dispatchers.Default)`) without losing the transaction context, allowing CPU-bound work to be
offloaded while keeping the transaction active.
...
transaction(propagation = REQUIRED) {
    ...
    transaction(propagation = NESTED) {
        ...
        if (!discountService.processDiscount(order, promo)) {
            setRollbackOnly()                                   // Roll back only the discount changes.
        }
    }
    val adjustedOrder = orderRepository.findById(order.id)    // Fetch with potential discount applied.
