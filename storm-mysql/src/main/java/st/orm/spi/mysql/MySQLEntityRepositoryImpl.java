...
public class MySQLEntityRepositoryImpl<E extends Record & Entity<ID>, ID>
    ...
    private String getVersionString(@Nonnull Column column) {
        ...
        return STR."\{columnName} = \{updateExpression}";
    }

    private StringTemplate onDuplicateKey(@Nonnull AtomicBoolean versionAware) {
        ...
        model.columns().stream()
                ...
                .map(column -> {
                    ...
                    if (column.autoGenerated()) {
                        return STR."\{columnName} = LAST_INSERT_ID(\{columnName})";
                    }
                    return STR."\{columnName} = VALUES(\{columnName})";
                })
                .forEach(values::add);
        model.columns().stream()
    ...
    public void upsert(@Nonnull E entity) {
        ...
        try (var _ = intercept(sql -> sql.versionAware(versionAware.getPlain()))) {
             var query = ormTemplate.query(flatten(RAW."""
                    ...
                    VALUES \{entity}\{onDuplicateKey(versionAware)}"""));
            query.executeUpdate();
        }
    }
    ...
    public ID upsertAndFetchId(@Nonnull E entity) {
        ...
        try (var _ = intercept(sql -> sql.versionAware(versionAware.getPlain()));
             var query = ormTemplate.query(flatten(RAW."""
                    INSERT INTO \{model.type()}
                    VALUES \{entity}\{onDuplicateKey(versionAware)}""")).prepare()) {
            query.executeUpdate();
    ...
    protected PreparedQuery prepareUpsertQuery() {
        ...
        try (var _ = intercept(sql -> sql.versionAware(versionAware.getPlain()))) {
            return ormTemplate.query(flatten(RAW."""
                    INSERT INTO \{model.type()}
                    VALUES \{bindVars}\{onDuplicateKey(versionAware)}""")).prepare();
        }
    }
