...
/**
 * Runs the extension against Oracle in a Testcontainers-managed container: the scripts execute in a database
 * provisioned for the class, and per-test rollback works as it does on H2.
 */
@StormTest(database = ORACLE, scripts = {"/container-schema.sql", "/container-data.sql"})
class StormExtensionOracleTest {
    ...
    @Test
    void connectionShouldBeOracle(DataSource dataSource) throws Exception {
        try (var connection = dataSource.getConnection()) {
            assertEquals("Oracle", connection.getMetaData().getDatabaseProductName());
            assertEquals(23, connection.getMetaData().getDatabaseMajorVersion());
        }
    }
