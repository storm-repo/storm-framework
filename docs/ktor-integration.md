...
Since 1.12, no registration step is needed. When the plugin is installed, Storm reads the compile-time type index generated by the metamodel processor (KSP or annotation processor, already part of the standard Storm setup) and registers every repository interface it finds. No runtime classpath scanning is involved. `repository<T>()` then works anywhere:
...
If your project uses Koin for dependency injection, expose Storm's registry to Koin using `forEach`. With auto-registration (the default), the registry already contains every indexed repository, so an empty `stormRepositories { }` block is enough to obtain it:
