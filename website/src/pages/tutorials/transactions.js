...
<div class="art">
  ...
  <p>If you run Spring, the two systems bridge, on one condition. With <code>@EnableTransactionIntegration</code> (the Kotlin Spring Boot starter enables it by default), Storm's blocks delegate to Spring's transaction manager: a <code>transactionBlocking</code> inside a <code>@Transactional</code> method joins the surrounding transaction, and propagation behaves as one system. Without that bridge, the two managers are independent, and a block inside a <code>@Transactional</code> method would open its own connection and its own transaction.</p>
