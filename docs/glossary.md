...
**SQL Scope**
A recording opened around a unit of work, such as one request, reporting what it cost the database: how many statements ran, how long the database spent on them against how long the call took, how far the work overlapped, and which statement carried the weight. Summaries carry no parameter values, so they are safe to log in production, where thresholds report only calls that exceed them. See [SQL Logging](sql-logging.md#per-call-summaries).
...
**Statement Origin**
Classifies what caused a statement to execute: `DIRECT` for statements the code asked for through a repository, query builder, or template, and `FETCH` for statements resolving a reference through `Ref.fetch()`. The origin appears on the `storm.origin` metric tag, in SQL scope summaries, and on captured statements in tests, making the cost of resolving references measurable on its own. See [SQL Logging](sql-logging.md#fetches).
