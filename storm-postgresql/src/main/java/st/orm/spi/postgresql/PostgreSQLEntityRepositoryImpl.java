...
public class PostgreSQLEntityRepositoryImpl<E extends Record & Entity<ID>, ID>
    ...
    private TemplateString getVersionString(@Nonnull Class<? extends Record> type, @Nonnull Column column) {
        TemplateString columnName = TemplateString.raw(column.qualifiedName(ormTemplate.dialect()));
        TemplateString updateExpression = switch (column.type()) {
            ...
            case Class<?> c when Instant.class.isAssignableFrom(c)
                    ...
                    || Timestamp.class.isAssignableFrom(c) -> TemplateString.raw("CURRENT_TIMESTAMP");
            default ->
    ...
    private TemplateString onConflictClause(@Nonnull AtomicBoolean versionAware) {
        ...
        var assignments = model.columns().stream()
                ...
                .map(column -> {
                    ...
                    return TemplateString.raw(STR."\{column.qualifiedName(dialect)} = EXCLUDED.\{column.qualifiedName(dialect)}");
                })
                .reduce((left, right) -> combine(left, TemplateString.raw(", "), right))
                .map(st -> combine(TemplateString.raw("DO UPDATE SET "), st))
                .orElse(TemplateString.raw("DO NOTHING"));
        return flatten(combine(TemplateString.raw("\nON CONFLICT ("), TemplateString.raw(conflictTarget), TemplateString.raw(") \0", assignments)));
    }
