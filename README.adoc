...
The Storm API provides a powerful and flexible way to query entities. It supports both a DSL mode and a SQL Template
mode, allowing developers to choose the approach that best fits their needs.

The following example demonstrates how to query the `User` entity using both DSL mode and SQL Template mode. The query
...
DSL mode should generally be preferred for its type-safe, readable syntax and portability. However, SQL Template mode is
...
between DSL and SQL Template modes.
...
record User(@PK Integer id,
            ...
            String postalCode,
            @FK City city,
            @Nullable @FK Ref<User> invitedBy
) implements Entity<Integer> {}
----
Kotlin::
+
...
underlying database field is null, the `Ref` field will be `null`.
...
Fetching entity data through Refs is always explicit, which means there are no hidden database queries that could lead
to N+1 query problems. For any given Ref instance, calling `fetch()` will query the database at most once; subsequent
calls return the previously fetched entity. When a result set contains multiple Refs pointing to the same entity, the
underlying data is typically loaded only once. Additionally, within the scope of a transaction, calling `fetch()` on a
Ref may return an entity that was already loaded earlier, without requiring another database query. This behavior may
vary depending on the transaction isolation level and other factors.

==== Creating Refs with Ref.of()

The `Ref.of()` factory methods allow you to create Ref instances programmatically. This is useful when you have an
entity or its primary key and need to create a Ref for use in queries, as map keys, or for passing to other parts of
your application.
...
[source,java]
----
// Create a Ref from a type and primary key
Ref<User> userRef = Ref.of(User.class, 42);

// Create a Ref from an existing entity
User user = ...;
...
Refs created with `Ref.of()` are detached, meaning they are not connected to a database context. Calling `fetch()` on
a detached Ref created from just a type and primary key will fail, since there is no database connection available.
However, a Ref created from an existing entity will return that entity when `fetch()` is called.
...
for both **suspend functions** (coroutine-friendly, non-blocking access) and **blocking functions**.
...
----
Programmatic Blocking (Kotlin)::
+
[source,kotlin]
----
transactionBlocking(isolation = REPEATABLE_READ, timeoutSeconds = 30) {
    val orders = orderRepository.findPendingOrders()
...
=== Update Modes and Dirty Checking

Storm entities are stateless by design. They are represented as Java records or Kotlin data classes, and Storm does not
maintain a long-lived persistence context. Dirty checking is therefore scoped to the lifetime of a single transaction
and is based on observing entity state rather than tracking mutations on managed objects.

Within a transaction, Storm observes all entities that are read, regardless of whether they were loaded via an
EntityRepository, a DSL query, or a SQL template. This observed state is used as the baseline for dirty checking.
...
Dirty checking is evaluated for all *updatable columns* of an entity. Fields that do not map to updatable columns
are ignored.

Nested record components are inspected transitively. Comparisons are performed for all record components that
contribute to updatable columns, including fields inside nested records, rather than treating nested records as a
single unit.

When available, generated metamodel implementations (Java annotation processing or Kotlin KSP) are used to perform
type-specific equality checks. This avoids reflective access and prevents boxing for primitive fields. If no generated
implementation is available, reflective access is used while still applying type-appropriate comparisons.

==== Update modes

Storm supports three update modes:

* OFF
* ENTITY
* FIELD

Update modes determine how dirty information is interpreted and how UPDATE statements are generated. They primarily
affect performance characteristics and write behavior. They do not provide correctness guarantees under concurrency.
To reliably detect conflicting updates, optimistic locking using a version column is still required.

The default update mode is ENTITY. This mode provides a good balance between predictability, batching efficiency, and
avoiding unnecessary database writes. The default mode can be changed by setting the `storm.update.defaultMode`
system property.

==== UpdateMode.OFF

When UpdateMode.OFF is used, dirty checking is completely disabled.

Every call to update() results in an UPDATE statement that writes all mapped, updatable columns.
Storm does not compare the current entity state with any previously observed state.

This mode offers:

* fully predictable behavior
* a single, stable SQL statement shape
* optimal batching characteristics
* minimal runtime overhead

It is a good fit for simple workloads or scenarios where write volume is low and avoiding redundant updates is not a
concern.

==== UpdateMode.ENTITY (default)

UpdateMode.ENTITY enables full-row updates with column-level dirty detection.

Storm compares the current entity against the state that was observed when the entity was read in the same
transaction. The comparison is performed according to the configured dirty checking strategy and is evaluated across
all updatable columns, including those contributed by nested records.

If no difference is detected, the UPDATE is skipped entirely. If any difference is detected, an UPDATE is issued
that includes all mapped, updatable columns.

This mode has several important properties:

* unnecessary database writes are avoided
* SQL statement shape remains stable, preserving batching efficiency
* write contention is often reduced by skipping redundant updates
* the number of updated columns is not reduced

Because of this balance, ENTITY is the recommended default for most applications.

==== UpdateMode.FIELD

UpdateMode.FIELD enables field-level updates with column-level dirty detection.

In this mode, Storm compares individual updatable columns against the values that were observed when the entity was
read and generates an UPDATE statement that includes only the modified columns, as determined by the configured dirty
checking strategy.

This includes columns derived from nested record components. Changes inside nested records are detected at the level
of the columns they contribute to.

This can reduce write amplification and limit lock scope when different transactions modify different fields.

These improvements are best-effort and situational. Field-level dirty checking does not guarantee correctness.
Optimistic locking is still required to reliably detect conflicting updates.

==== Enabling field-level updates with @DynamicUpdate

Field-level updates are enabled per entity using the @DynamicUpdate annotation.
...
Storm supports two dirty checking strategies:

* instance-based dirty checking
* value-based dirty checking

By default, Storm uses instance-based dirty checking. A field is considered unchanged when its extracted value is
identical to the one observed at read time. A field is considered dirty as soon as its extracted value is no longer
identical.

This approach provides predictable and bounded performance characteristics and avoids potentially expensive value
comparisons for large objects.

Value-based dirty checking can be enabled explicitly. In this mode, fields are compared using semantic equality after
a fast identity check when applicable. A field is only considered dirty when its value has changed.

For entity-typed fields, instance-based dirty checking uses instance identity. Value-based dirty checking compares the
mapped values, such as the referenced identifier.

The dirty checking strategy can be configured globally using the system property `storm.update.dirtyCheck`, or per
entity using the `@DynamicUpdate` annotation.

==== Max shapes

Field-level updates can produce multiple SQL statement shapes, depending on which columns are modified. To prevent
unbounded growth in prepared statements and to preserve batching efficiency, Storm enforces a max shapes limit per
entity.

Storm tracks the distinct update shapes generated for an entity. Once the number of shapes exceeds the configured
limit, Storm automatically falls back to a full-row update for that entity. This ensures that UpdateMode.FIELD
remains safe to use even when update patterns are unpredictable.

The max shapes limit can be overridden by setting the system property `storm.update.maxShapes`. The default is 5.
