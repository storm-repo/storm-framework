...
- The slow statement log reports each single execution whose database time exceeds a threshold, under `st.orm.sql.slow` at `WARN`, wherever it runs: `storm.sql-log.slow-statement=200ms` in Spring, `sqlLogSlowStatement` / `storm.sqlLog.slowStatement` in Ktor, `storm.sql_log.slow_statement` on a plain JVM, independent of the per-call summaries. The decision is made where the statement returns from the database, so the call site is walked for slow executions only, and the line carries the statement, the rows, and how the execution compares to what its shape typically costs (`typically 6.0 ms, 306x`) and typically binds (`parameters 32 (typically 3)`), which says whether to look at the parameters or at the query; values render at `TRACE` only, lines are rate-limited per shape (`slow-statement-limit`, per minute), and a failed execution is reported with its failure class. Database time is now measured to the statement's return everywhere: the summary's `in database`, `SqlCapture.duration` and the slow line agree, and consuming a streamed read no longer counts as database time. Summary rows carry `max` when one execution stands out from the group's average, and a read whose stream is still open when the scope closes is in the summary with its database time and the rows read so far, where it used to be reported as not recorded.
