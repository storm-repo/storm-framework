...
import static st.orm.test.TestDatabase.MSSQL_SERVER;
...
/**
 * Runs the extension against Microsoft SQL Server in a Testcontainers-managed container: the scripts execute in a database
 * provisioned for the class, and per-test rollback works as it does on H2.
 */
@StormTest(database = MSSQL_SERVER, scripts = {"/container-schema.sql", "/container-data.sql"})
class StormExtensionMsSqlServerTest {
    ...
    @Test
    void connectionShouldBeMsSqlServer(DataSource dataSource) throws Exception {
