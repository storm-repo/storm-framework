...
/**
 * Runs the extension against MySQL in a Testcontainers-managed container: the scripts execute in a database
 * provisioned for the class, and per-test rollback works as it does on H2.
 */
@StormTest(database = MYSQL, scripts = {"/container-schema.sql", "/container-data.sql"})
class StormExtensionMySqlTest {
    ...
    @Test
    void connectionShouldBeMySql(DataSource dataSource) throws Exception {
