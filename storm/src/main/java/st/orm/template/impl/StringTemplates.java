...
public final class StringTemplates {
     ...
     */
    public static StringTemplate flatten(@Nonnull StringTemplate template) {
        FlattenResult result = flattenTemplate(template);
        return StringTemplate.of(result.fragments, result.values);
    }

    private record FlattenResult(List<String> fragments, List<Object> values) {}

    private static FlattenResult flattenTemplate(@Nonnull StringTemplate template) {
        List<String> tmplFragments = template.fragments();
        List<Object> tmplValues = template.values();
        if (tmplValues.stream().noneMatch(v -> v instanceof StringTemplate)) {
        ...
        for (int i = 0; i < substitutionCount; i++) {
            ...
            if (sub instanceof StringTemplate nestedTemplate) {
