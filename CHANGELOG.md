...
- `andAny` and `orAny` are removed too, and `and`/`or` inherit the query root. In Kotlin they live in the `whereBuilder { }` scope, where a join widens the root so the same expression combines predicates across joined entities. Outside a scope, the top-level `and`/`or` extensions root the combination at the operands' least common root: written directly in a `where(...)` call, the builder's root drives the inference, so a narrow builder rejects a cross-root combination at the call site while a widened builder accepts it and verifies the paths when the query is built.
...
Applications program against the facade API; the engine that executes it is a runtime concern. The artifacts now enforce what the installation guide always documented: `storm-core` reaches the application at runtime only, so completion and imports offer exactly one `EntityRepository`, one `ORMTemplate`, one `QueryBuilder`: the facade's.

- The application-level integration contract moved out of the engine into `storm-foundation`, under the `st.orm.spi` package: exception translation (`ExceptionMapper`, `ExceptionContext`, `SqlOperation`), query observation (`QueryObserver`, `QueryContext`, `StatementOrigin`), SQL commenting (`SqlCommenter`) and cursor codecs (`CursorCodecProvider`, `CursorCodec`, `CursorCodecEntry`). `SqlTemplateException` joins `PersistenceException` in `st.orm`. These are the types the facade builder, the Ktor plugin configuration and the Spring integration hand to applications to implement, alongside `EntityCallback` and `TemplateDecorator`, so they need no dependency beyond the facade; implementations update their imports, and `META-INF/services/st.orm.core.spi.CursorCodecProvider` registrations rename to `st.orm.spi.CursorCodecProvider`. Transaction bridging (`ConnectionProvider`, `TransactionTemplateProvider`, `ExternalTransactionProvider` and the transaction types they exchange), `RefFactory` and the dialect surface are engine SPI and stay in `storm-core`.
- The Java and Kotlin facade builders drop `connectionProvider(...)` and `transactionTemplateProvider(...)`: bridging a transaction manager is framework-level composition that Storm's Spring modules perform on the engine's own builder, exposed as `SpringOrmTemplate.builder(dataSource, beanFactory)` (Java) and `springOrmTemplateBuilder(dataSource, beanFactory)` (Kotlin) for templates composed by hand, so the application-facing builder keeps only options an application can name: `config`, `decorator`, `manualCommitConnections`, `exceptionMapper`, `queryObserver` and `sqlCommenter`.
- The Spring Boot starters carry `storm-core` at runtime scope; the library modules already declared it `provided`. A project that follows the installation guide, `implementation` on the facade and `runtimeOnly` on `storm-core`, now genuinely compiles without the engine on the classpath.
- On the module path the facades require `storm-foundation` transitively, so `Ref`, `Metamodel` and the `st.orm.spi` contract resolve without extra `requires` directives. `storm.core` stays non-transitive to applications, and its repository package is exported to Storm's own modules only.
- `QueryObserver.onTransaction` receives the `st.orm.TransactionOptions` an application passes to `transaction(options) { }`; the scope-internal `TransactionScope.Options` record is gone, and whether a transaction runs in suspend mode stays a detail of the scope. `CacheRetention.fromConfig` moved into the engine's configuration helper.
- Creating a template without the engine on the runtime classpath fails immediately, naming the missing `st.orm:storm-core` dependency and how to add it, rather than surfacing as a `NoClassDefFoundError` at the first statement.
- `@StormTest` injects `SchemaValidation`, a facade over the engine's schema validator that reports mismatches as rendered messages; the engine's `SchemaValidator` is no longer an injectable parameter type, keeping test classpaths engine-free as well.
...
- The slow statement log reports each single execution whose database time exceeds a threshold, under `st.orm.sql.slow` at `WARN`, wherever it runs: `storm.sql-log.slow.threshold=200ms` in Spring, `sqlLogSlowThreshold` / `storm.sqlLog.slow.threshold` in Ktor, `storm.sql_log.slow.threshold` on a plain JVM, independent of the performance log. The decision is made where the statement returns from the database, so the call site is walked for slow executions only, and the line carries the statement, the rows, and how the execution compares to what its shape typically costs (`typically 6.0 ms, 306x`) and typically binds (`parameters 32 (typically 3)`), which says whether to look at the parameters or at the query; values render at `TRACE` only, lines are rate-limited per shape (`slow-statement-limit`, per minute), and a failed execution is reported with its failure class. Database time is now measured to the statement's return everywhere: the summary's `in database`, `SqlCapture.duration` and the slow line agree, and consuming a streamed read no longer counts as database time. Summary rows carry `max` when one execution stands out from the group's average, and a read whose stream is still open when the scope closes is in the summary with its database time and the rows read so far, where it used to be reported as not recorded.
- The SQL log is configured per log rather than in one flat section, and named after the two loggers it writes to. `storm.sql-log.performance.*` carries the performance log (`st.orm.sql.perf`), which says what a unit of work cost the database; `storm.sql-log.slow.*` carries the slow statement log (`st.orm.sql.slow`), which names the execution that cost too much. `enabled`, `limit`, `call-sites`, `line-width`, `entry-points` and `threshold.*` move under `performance`; `slow-statement` and `slow-statement-limit` become `slow.threshold` and `slow.limit`; `call-site-skip` stays at the top, since both logs attribute a frame the same way. The Ktor plugin follows in both the DSL (`sqlLogPerformance`, `sqlLogPerformanceLimit`, `sqlLogSlowThreshold`, ...) and the HOCON keys, and the system properties become `storm.sql_log.performance.line_width`, `storm.sql_log.slow.threshold` and `storm.sql_log.slow.limit`. What `enabled` switched on was never the whole SQL log, only one of the two.
- The Spring types follow the same split: `StormSqlLogFilter` and `StormSqlLogEntryPointPostProcessor` are `StormPerformanceLogFilter` and `StormPerformanceLogEntryPointPostProcessor`, and the shared reporting helper is `PerformanceLog`. `SqlLog.Summary` keeps its name: a summary is what the performance log reports, not the log itself.
- The SQL log is retunable while the application runs, so what a deployment reports is a decision that can be made about the deployment that is misbehaving rather than one made months earlier. Every setting either half reports with is read per unit of work: `SlowStatementLog.threshold(Duration)` and `SlowStatementLog.limit(int)` land on the next execution, and the performance log's boundaries read their thresholds, their call-site attribution and their record limit per request or invocation. In Spring, where the actuator is on the class path, the `storm` endpoint reads and sets both halves (`GET`/`POST /actuator/storm`, with `off` to remove a threshold), exposed like any other endpoint and belonging behind the same authorization as the other write endpoints. The endpoint is Storm's control surface rather than the SQL log's, one section per subject, so what Storm learns to be told at runtime later joins it instead of claiming an endpoint of its own. `storm.sql-log.performance.enabled` stays a startup decision: it installs the request filter and the entry-point proxies, which a refreshed context cannot be given.
- The slow statement log takes its threshold from the performance log when it has none of its own: `storm.sql-log.slow.threshold` unset alongside `storm.sql-log.performance.enabled=true` follows `storm.sql-log.performance.threshold.duration` (`sqlLogPerformanceDurationThreshold` in Ktor). A call that exceeds a duration holds at least one execution, so a statement threshold no lower than the call's only fires inside a call that is reported anyway: the derived default names the statement behind a warning instead of adding warnings of its own. An explicit `slow-statement` still wins, and with neither set there is no slow log.
...
- The slow statement log's per-shape rate limit covers the executions it has no shape for. A statement whose shape cannot be derived, and every shape past the four thousand tracked, used to skip the limit entirely and report on every slow execution, so the limit stopped applying exactly where the log knew the least and a degraded database could flood it. Those executions now share one reporting budget; they still carry no baseline, since there is none to carry.
- The statement's shape identity uses the full 64 bits it is declared with. It was folded through a 32-bit hash and widened to a `long`, so a shape whose hash was negative reported a sign-extended `ffffffff` prefix that says nothing about the shape, at twice the width of one whose hash was positive. Shapes are derived from the key's content now and render as a fixed sixteen hex digits, both in the slow statement log and in the `storm.shape` observation tag. Identities differ from earlier releases, so dashboards that pin a shape value need repointing.
- The Spring Boot starters expose `OrmTemplateFactory`: one bean that composes a fully integrated `ORMTemplate` wherever the application defines its own template beans, which backs the auto-configured single template off. Each created template carries the same property-driven integration as the auto-configured one. SQL failure translation follows `storm.exception-translation.enabled` with the mapper created against each template's own data source; observations resolve their semantic conventions from each data source's JDBC URL and report the template's name as the low-cardinality `storm.database` key value, composed through the new `StormQueryObservers`, which the observation auto-configuration also builds the single template's observer with. A customize block applies application-specific composition, such as a table name resolver, without the application touching the integration SPI.
- The Ktor plugin closes its two composition gaps against the starters. `storm.observations.semanticConventions = otel` selects the OpenTelemetry database semantic conventions from configuration, resolved per database from its own data source and overridable under `storm.databases.<name>.observations.semanticConventions`; previously a convention instance had to be registered in the dependency container and served every database with one product. A `customize` slot on the plugin and per database block applies application-specific composition, such as a table name resolver, after the integration is wired, inheriting the plugin-level block unless the database sets its own. Observer composition is shared with the Spring Boot starters through `QueryObservers` in storm-micrometer, so the stacks resolve conventions and the `storm.database` identity the same way.
...
- `SqlLog` carries the diagnostics API only: summary rendering lives in `SqlLogRenderer`, call-site capture in `CallSiteCapture`, both internal. How summaries render is configured through the `storm.sql_log.*` system properties or the corresponding Spring and Ktor keys.
- Removed the hydration shape from SQL log summary rows, with `storm.sql-log.hydration`, `sqlLogHydration`, `storm.sql_log.hydration` and `SqlLog.HydrationShapes`: a type's declared shape is a per-type fact, and printing it per execution behind an option was decoration. The per-type report belongs to `storm analyze` (#503).
...
- The Java and Kotlin facades close their remaining parity gaps against each other. Kotlin's path-based `where` and `whereRef` clauses, on the builder and in `WhereBuilder`, accept `Navigable` paths, so navigation-only nodes reached through a `Ref` compile in both languages; Java's `where(path, record)` bounds the record by `Data`, as the other surfaces always did. Kotlin's `Templates` gains the named `param(name, Calendar, TemporalType)` variant.
- The `select { }` and `delete { }` blocks add the remaining clause forms: `where(records)`, `where(path, records)`, `where(path, operator, values)`, `whereRef(path, refs)` and the descending order template. `whereRef` now also works inside `whereBuilder { }`: the ref parameters are covariant, following `where(ref)`.
- Kotlin's `findRefBy` drops two unused type parameters that forced explicit type arguments onto call sites, and `findAllRefBy(field, values)` accepts any value type, matching Java. Both are source-incompatible only for call sites that spelled the old type arguments out.
- The query builder's `hasOrderBy()` probe is no longer public API; it leaked on the Kotlin facade while the Java facade kept it protected. Combining explicit `orderBy` with `Pageable` or `Scrollable` sorting still fails with a descriptive error.
- The two-argument `selectFrom(fromType, selectType)` renders the select list from the FROM table on all three surfaces, so `selectType` may be any record shape over those columns, such as a wrapper record nesting the from-entity. The Java and Kotlin facades rendered `selectType` instead, which rejected exactly those wrapper records.
