...
final class ModelFactory {
            ...
            return (Model<T, ID>) MODEL_CACHE.computeIfAbsent(type, _ -> {
                ...
                List<Column> columns = Stream.of(type.getRecordComponents())
                        .flatMap(c -> {
                            ...
                            return createColumns(builder, c, primaryKey, autoGenerated, index).stream();
                        })
                        .toList();
                try {
                    String tableName = getTableName(type, builder.tableNameResolver());
                    //noinspection unchecked
    ...
    private static <T extends Record> List<Column> createColumns(@Nonnull ModelBuilder builder, @Nonnull RecordComponent component, boolean primaryKey, boolean autoGenerated, @Nonnull AtomicInteger index) {
        try {
            var converter = getORMConverter(component).orElse(null);
            if (converter != null) {
                var columnTypes = converter.getParameterTypes();
                var columnNames = converter.getColumns(c -> getColumnName(c, builder.columnNameResolver()));
                ...
                for (int i = 0; i < columnTypes.size(); i++) {
                    columns.add(new Column(index.getAndIncrement(), columnNames.get(i), columnTypes.get(i), primaryKey,
            ...
            if (foreignKey) {
                ...
                if (componentType.isRecord()) {
                    ...
                    return Stream.of(componentType.getRecordComponents())
                            .flatMap(c -> createColumns(builder, c, primaryKey, autoGenerated, index).stream())
                            .toList();
                }
            ...
            return List.of(new Column(index.getAndIncrement(), columnName, componentType, primaryKey, autoGenerated, foreignKey, nullable, insertable, updatable, version, lazy));
        } catch (SqlTemplateException e) {
            throw new PersistenceException(e);
        }
    }
