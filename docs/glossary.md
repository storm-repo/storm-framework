...
**Slice**
A page without the count query. `slice(pageable)` takes the same `Pageable` as `page`, reads one row beyond the page size to decide `hasNext`, derives `hasPrevious` from the page number, and navigates with `next()` and `previous()`. The read for a "load more" that needs no total, and for a query without a unique key, where scrolling is not possible.
