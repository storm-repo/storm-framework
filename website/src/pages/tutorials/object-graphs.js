...
<div class="art">
  ...
  <p>The owner was never passed. <code>insert</code> extends the entities you supply with every unsaved entity reachable through their foreign key fields, the <em>insertion closure</em>. Storm orders the result by foreign key dependencies, executes one batch per entity type per level, and propagates generated keys by instance identity: both pets hold the same <code>owner</code> instance, so both rows receive the same key. Nothing on <code>Owner</code> says that pets depend on it; the pet values say so themselves.</p>
  ...
  <p>The other verbs follow the same contract. Where <code>merge</code> with <code>CascadeType.MERGE</code> walks the associations and decides insert-or-update from entity state, Storm splits the decision into explicit verbs: <code>update</code> for rows you know exist, <a href="/docs/upserts">upsert</a> when the database should resolve it atomically. Both write exactly the entities you pass, and <code>update</code> rejects unsaved members instead of quietly inserting them.</p>
