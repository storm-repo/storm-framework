...
public class EntityRepositoryImpl<E extends Entity<ID>, ID>
    ...
    public List<ID> insertAndFetchIds(@Nonnull Iterable<E> entities) {
        ...
        if (isAutoGeneratedPrimaryKey() && ormTemplate.dialect().supportsMultiRowGeneratedKeys()) {
     ...
     * generated primary keys in insertion order. Used when the dialect
     * {@linkplain st.orm.core.template.SqlDialect#supportsMultiRowGeneratedKeys() returns all generated keys} for a
     * multi-row insert. Unlike the {@code executeBatch} path, this issues one round trip and lets the driver return
     * every key at once, avoiding the batch rewrite the driver disables when generated keys are requested.
     */
