package st.orm.json.model;

import jakarta.annotation.Nonnull;
import lombok.Builder;
import st.orm.FK;
import st.orm.DbName;
import st.orm.PK;
import st.orm.Persist;
import st.orm.repository.Entity;

@Builder(toBuilder = true)
@DbName("vet_specialty")
public record VetSpecialty(
        @Nonnull @PK(autoGenerated = false) VetSpecialtyPK id,  // Implicitly @Inlined
        @Nonnull @Persist(insertable = false) @FK @DbName("vet_id") Vet vet,
        @Nonnull @Persist(insertable = false) @FK @DbName("specialty_id") Specialty specialty) implements Entity<VetSpecialtyPK> {
    public VetSpecialty(@Nonnull VetSpecialtyPK pk) {
        //noinspection DataFlowIssue
        this(pk, null, null);
    }
}
