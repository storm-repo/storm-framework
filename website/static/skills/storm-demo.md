...
Use Flyway for schema management. Add the Flyway dependency. The migration goes in `src/main/resources/db/migration/V1__create_schema.sql`. The same SQL file should also be copied to `src/test/resources/schema.sql` for use by `@StormTest`. Keep the migration H2-compatible so the per-step verification tests stay on H2, which is what keeps them fast and visible. If the migration needs DDL H2 rejects, or as a closing step once the application is complete, run the test suite once on the demo's own database: set `database = TestDatabase.POSTGRESQL` (or the constant matching the configured database: `MYSQL`, `MARIADB`, `MSSQL_SERVER`, `ORACLE`) on the `@StormTest` classes and add the database's Testcontainers module (`org.testcontainers:testcontainers-postgresql` and so on; see /storm-setup for the Testcontainers 1 names) plus its driver in test scope; Docker is available because Docker Compose runs the database. Narrate what that pass proves: the same tests, on the real dialect.
