...
- The SQL log is retunable while the application runs, so what a deployment reports is a decision that can be made about the deployment that is misbehaving rather than one made months earlier. Every setting either half reports with is read per unit of work: `SlowStatementLog.threshold(Duration)` and `SlowStatementLog.limit(int)` land on the next execution, and the performance log's boundaries read their thresholds, their call-site attribution and their record limit per request or invocation. In Spring, where the actuator is on the class path, the `stormsqllog` endpoint reads and sets both halves (`GET`/`POST /actuator/stormsqllog`, with `off` to remove a threshold), exposed like any other endpoint and belonging behind the same authorization as the other write endpoints. `storm.sql-log.performance.enabled` stays a startup decision: it installs the request filter and the entry-point proxies, which a refreshed context cannot be given.
