...
=== Dirty Checking and Update Modes
...
However, dirty checking is only applied when updates are executed through an EntityRepository. Manual SQL updates,
...
Update modes primarily influence performance characteristics and write behavior. They do not provide correctness
guarantees under concurrency. To reliably detect conflicting updates, optimistic locking using a version column is still
required.

The default update mode is ENTITY, which provides a good balance between predictability, batching efficiency, and
avoiding unnecessary database writes. The default mode can be changed by setting the "storm.update.defaultMode" system
property.
...
transaction. If no difference is detected, the UPDATE is skipped entirely. If any difference is detected, Storm issues
an UPDATE that includes all mapped columns.
...
UPDATE statement that includes only the modified columns. This can reduce write amplification and limit lock scope,
which often improves behavior under concurrent updates when different transactions modify different fields.

These improvements are best-effort and situational. Field-level dirty checking improves concurrency characteristics but
does not guarantee correctness. Optimistic locking is still required to reliably detect conflicting updates.
...
Storm tracks the distinct update shapes generated for an entity. Once the number of shapes exceeds the configured limit,
Storm automatically falls back to a full-column update for that entity. This ensures that UpdateMode.FIELD remains safe
to use even when update patterns are unpredictable.

The max shapes limit can be overridden by setting the system property "storm.update.maxShapes". The default is set to 5.
