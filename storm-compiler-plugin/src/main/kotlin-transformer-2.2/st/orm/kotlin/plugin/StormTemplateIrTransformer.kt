    ...
    private inner class ResultConstantRewriter(
        ...
        private fun rewriteCallReceivers(call: IrCall) {
            ...
            for (index in parameters.indices) {
                val kind = parameters[index].kind
                if (kind != IrParameterKind.DispatchReceiver && kind != IrParameterKind.ExtensionReceiver) continue
                val argument = call.arguments.getOrNull(index) ?: continue
                call.arguments[index] = rewriteResultExpression(argument)
            }
        }
    }
