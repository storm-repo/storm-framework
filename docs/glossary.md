...
**Slice**
A page of query results returned by keyset pagination methods (`slice`, `sliceAfter`, `sliceBefore`). A `Slice<E>` contains the result list (`content`) and a `hasNext` flag indicating whether more results exist. Unlike offset-based pagination, keyset pagination performs consistently regardless of page depth. See [Queries](queries.md#keyset-pagination-with-slice).
