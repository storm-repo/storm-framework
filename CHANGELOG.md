...
- The `select { }` and `delete { }` blocks carry the chained builder's clause vocabulary in full: `whereId`, `whereRef`, `forLock`, the aliased `join(JoinType, ..., alias)` variants, and the template, subquery and cross joins. Type transitions (`narrow`, `widen`, `typedId`) and terminals deliberately stay on the builder the block returns: the scope delegates clauses to a widened builder while its own type parameters keep record, id, ref and fetch matching typed to the root, so inside the block there is nothing to narrow or widen.
...
- A Storm-managed transaction that receives a connection with auto-commit disabled fails with a message naming the two possible causes, a pool configured for manual commit or a connection carrying an unfinished transaction, and the fix, instead of stating only the precondition.
- `@StormTest` runs each test inside a database transaction that is rolled back afterwards, so tests no longer observe each other's writes and count assertions can be exact regardless of execution order. Every connection a test obtains shares that transaction, and Storm's transaction blocks demarcate with savepoints inside it, so a block commits and rolls back normally within the test while everything is still undone at the end. `rollback = false` opts a class out for tests that need real commits: writes visible to other connections, `REQUIRES_NEW` independence, or DDL inside a test.
- The Gradle plugin wires the metamodel processor into every source set's processor configuration — `testAnnotationProcessor`, `kspTest`, and custom source sets' equivalents — so entities declared in test sources get a metamodel; those configurations never extend each other, so the main-only wiring silently skipped them. Javadoc generation gets the same `--enable-preview` flag as compilation: javadoc embeds the javac front end, which rejects storm-java21's preview class files without it, so `./gradlew javadoc` failed on the Java path.
- Transaction observations go through an observation convention, `StormTransactionObservationConvention`, the way query observations always did, so they can be renamed and re-tagged: supply a custom convention to `MicrometerQueryObserver`, contribute the `ObservationConvention<StormTransactionObservationContext>` bean in the Spring starters, or register it in the Ktor dependency container. The Ktor plugin's automatic binding forwarded queries only, so Ktor applications silently emitted no `storm.transaction` observations at all; it forwards transactions too now.
- Query observations carry the statement's shape identity as the low-cardinality `storm.shape` key value: statements generated from one template share it whatever their parameters expand to, so grouping by shape charts one query template where the text would split it. The shape used to be derived only while a SQL log scope was open; it is derived unconditionally now, cached per compiled template.
- `StormConfig.sqlShapingKeys()` exposes the configuration keys whose values affect the generated SQL, derived from markers on the key declarations. The template cache key includes exactly these keys, so declaring a new SQL-shaping key is all it takes for configurations that differ in it to stop sharing cached SQL; the hand-maintained key list in the cache, one omission away from serving SQL built under a different configuration, is gone.
- A grouping states what one row stands for, and the generator emits the columns that express it. `groupBy` accepts the entity you want one row of — the relationship (`Pet_.owner`), the key beyond it (`Pet_.owner.id`) or the query root's own key (`Pet_.id`) all state the same identity, while any other path still groups by its value. Selecting an entity selects the tables its foreign keys reach, so a grouping usually has to cover more than the table it names: those keys are added, which cannot change a group because every foreign key is to-one and one owner has one city. How many columns an identity takes is the dialect's decision — PostgreSQL, MySQL, SQLite and H2 accept the key alone, SQL Server and Oracle require every selected column of that table — so the query is written once and runs everywhere. **This changes the SQL generated for existing aggregate queries.** An aggregate over an entity with a foreign key previously grouped one table while selecting several, which H2 accepts and PostgreSQL, SQL Server and Oracle reject, so such queries were valid only on the permissive databases; they now group every table they select. A grouping that determines nothing — selecting `Pet` while grouping `Pet_.owner`, where one owner has many pets — is refused when the query is built, on every dialect rather than only the ones that would notice, because the permissive databases answer it with an arbitrary row. Groupings that already enumerate their columns keep working, with those columns now redundant.
- The public API is JSpecify null-marked: every Java package carries `@NullMarked`, nullable type uses carry `org.jspecify.annotations.Nullable`, and the jakarta annotations are gone from Storm's signatures, so `jakarta.annotation-api` is no longer a Storm dependency — projects that used it transitively must declare it themselves. JSpecify itself is an optional dependency: javac, the Kotlin compiler and the analysis tools read the annotations from bytecode by name, so nothing new lands on the consumer's classpath. Kotlin callers get real `T`/`T?` types where the Java surface used to be platform types, checked strictly by Kotlin 2.1+. User models keep their existing vocabulary — JSpecify, jakarta and javax nullness annotations on records are read reflectively as before — and generated metamodel sources adapt to the class path: with JSpecify present their nullable `getValue` returns carry a fully qualified type-use `@Nullable`, without it they reference no annotation library at all. Type parameters that admit nullable arguments declare it — `V extends @Nullable Object` on `TypedMetamodel`, `AbstractMetamodel` and `AbstractKeyMetamodel`, `@Nullable` elements on `Instantiator`'s argument arrays, `@Nullable` on `Metamodel.getValue`, `T extends @Nullable Object` on `SqlCapture.execute` and `executeThrowing`, whose action may end in a lookup that returns no row — so Kotlin 2.1+ consumers compile the generated nullable metamodel chain; strict JSpecify enforcement starts at Kotlin 2.1, which the reactor's own Kotlin 2.0 modules never exercise, so the Gradle plugin's Kotlin 2.4 smoke projects guard this surface.
- The `@DataStormTest` slice imports every auto-configuration the starters register in production, verified by a parity test so the two lists cannot drift again. Repository AOP proxying was missing, so `@Transactional` and other interface-level advice on repositories behaved differently under test than in the running application. The slice also provides `JdbcTemplate` and `JdbcClient` for verifying database state through a channel independent of the ORM under test, and Testcontainers' service connection support, so `@ServiceConnection` on a `@Bean`-declared container supplies the slice's database instead of silently doing nothing. Every slice import resolved from another artifact carries the `optional:` prefix, so a classpath without that artifact skips the entry instead of failing the import selector with a `ClassNotFoundException`, and the module no longer pins the consumer's JUnit version, which it needs only to compile the annotation.
- `Projection<ID>`'s type argument is a checked contract instead of a phantom parameter. `ID` is the projection's row identity type, the type the id-based operations work with (`findById`, `ref(id)`, `Ref.projectionId`); for a foreign-key-typed primary key that is the referenced table's key rather than the component value, which is why the interface deliberately declares no id accessor and `Ref.of(projection, id)` takes the id explicitly. Record validation rejects a declaration the record contradicts — `Projection<Void>` alongside a mapped `@PK`, or a declared type that differs from the key's row identity — while a projection without a mapped `@PK` may still declare its row identity, which serves typed detached refs. The rule that a foreign key must not be an auto-generated primary key applies to entities only now: generation is insert machinery and projections are never inserted, so `@PK @FK` on a projection no longer demands a meaningless `generation = NONE`.
- The deliberately paired artifacts, `storm-java21`/`storm-kotlin`, the two Spring Boot starters, `storm-jackson2`/`storm-jackson3` and the two metamodel processors, ship the same fully qualified names, one half per classpath. Each half bans its twin through a maven-enforcer rule, as do the modules that carry a half transitively, so a Storm dependency tree that picks up both halves of a pair fails the build with a message naming the conflict and the path that introduces it, instead of an application failing at first use to whichever jar wins classpath ordering. The one-per-classpath rule for all four pairs is documented centrally in the installation guide.
