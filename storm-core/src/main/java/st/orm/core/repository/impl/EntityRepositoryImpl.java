...
public class EntityRepositoryImpl<E extends Entity<ID>, ID>
    ...
    protected void insert(@Nonnull List<E> batch, @Nonnull Supplier<PreparedQuery> querySupplier) {
        insert(batch, querySupplier, false);
    }

    protected void insert(@Nonnull List<E> batch, @Nonnull Supplier<PreparedQuery> querySupplier, boolean ignoreAutoGenerate) {
        if (batch.isEmpty()) {
            return;
        }
    ...
    protected List<ID> insertAndFetchIds(@Nonnull List<E> batch, @Nonnull Supplier<PreparedQuery> querySupplier) {
        return insertAndFetchIds(batch, querySupplier, false);
    }

    protected List<ID> insertAndFetchIds(@Nonnull List<E> batch, @Nonnull Supplier<PreparedQuery> querySupplier, boolean ignoreAutoGenerate) {
        if (batch.isEmpty()) {
            return List.of();
        }
        var query = querySupplier.get();
    ...
    public void update(@Nonnull Stream<E> entities, int batchSize) {
        try (var query = prepareUpdateQuery()) {
            chunked(entities, batchSize)
                    .forEach(batch -> update(batch, () -> query));
        }
    }
    ...
    protected void update(@Nonnull List<E> batch, @Nonnull Supplier<PreparedQuery> querySupplier) {
        if (batch.isEmpty()) {
            return;
        }
        var query = querySupplier.get();
    ...
    protected List<ID> updateAndFetchIds(@Nonnull List<E> batch, @Nonnull Supplier<PreparedQuery> querySupplier) {
        if (batch.isEmpty()) {
            return List.of();
        }
        var query = querySupplier.get();
    ...
    protected List<E> updateAndFetch(@Nonnull List<E> batch, @Nonnull Supplier<PreparedQuery> query) {
