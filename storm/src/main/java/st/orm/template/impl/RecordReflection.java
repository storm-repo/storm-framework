...
final class RecordReflection {
    ...
    private static Optional<String> combine(@Nonnull String value, @Nonnull String name) {
        if (!value.isEmpty()) {
            if (!name.isEmpty() && !name.equals(value)) {
                throw new IllegalArgumentException(STR."Column name '\{name}' cannot be different from the column value '\{value}'.");
            }
    ...
    static ColumnName getColumnName(@Nonnull RecordComponent component,
        ...
        if (names.size() > 1) {
            throw new SqlTemplateException(STR."Multiple column names found for \{component.getDeclaringRecord().getSimpleName()}.\{component.getName()}: \{names}.");
        }
    ...
    private static List<ColumnName> getColumnNames(@Nonnull RecordComponent component,
        ...
        try {
            ...
            if (columNameLists.size() > 1) {
                throw new SqlTemplateException(STR."Multiple column names found for \{component.getDeclaringRecord().getSimpleName()}.\{component.getName()}: \{columNameLists}.");
            }
