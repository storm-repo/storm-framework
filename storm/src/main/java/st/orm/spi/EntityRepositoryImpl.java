...
public class EntityRepositoryImpl<E extends Entity<ID>, ID>
    ...
    public void deleteAll() {
        ORM."DELETE FROM \{model.type()}".executeUpdate();
    }
    ...
    public List<E> select(@Nonnull Iterable<ID> ids) {
        return select(toStream(ids)).toList();
    }
     ...
     */
    @Override
    public <R> R selectAll(@Nonnull ResultCallback<E, R> callback) {
        try (var stream = Tripwire.autoClose(this::selectAll)) {
     ...
     */
    @Override
    public <R> R select(@Nonnull Stream<ID> ids, @Nonnull ResultCallback<E, R> callback) {
    ...
    public Stream<E> select(@Nonnull Stream<ID> ids, int sliceSize) {
        return slice(ids, sliceSize, batch -> where(batch).stream());
    }
     ...
     */
    @Override
    public <R> R select(@Nonnull Stream<ID> ids, int batchSize, @Nonnull ResultCallback<E, R> callback) {
        try (var stream = Tripwire.autoClose(() -> select(ids, batchSize))) {
            return callback.process(stream);
        }
    }
        ...
        try (var query = ORM."""
            ...
            slice(entities, batchSize).forEach(batch -> {
                ...
                try (var generatedKeys = autoGeneratedPrimaryKey
                        ? Tripwire.autoClose(() -> query.getGeneratedKeys(model.primaryKeyType()))
                        : batch.stream().map(Entity::id)) {
